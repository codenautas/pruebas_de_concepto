...
create function opcion_no_numerica_trg() returns trigger 
  language plpgsql
as
$body$
begin
  new.no_numerica := no_numerica(new.opcion);
  return new;
end;
$body$;

create trigger opcion_no_numerica_trg
  BEFORE INSERT OR UPDATE ON variables_opciones 
    FOR EACH ROW
    EXECUTE FUNCTION opcion_no_numerica_trg();
